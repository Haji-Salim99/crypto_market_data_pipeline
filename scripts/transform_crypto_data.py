...
#Function for loading the latest json data from data/proccessed directory
...
#Function for categorizing market cap with labels
...
#Function for transforming the loaded raw json data
def transform_data():
    try:
        ...
        with open(raw_file_path, "r", encoding="utf-8") as file:
            ...
            logging.info(f"Raw data frame created with shape of : {df.shape}")
        ...
        if df["id"].isnull().any():
            logging.warning("Some records have missing coin IDs")
        ...
        logging.info(f"Total rows after transformation: {len(df)}")
